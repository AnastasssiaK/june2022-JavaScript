...
let y = 0;
while (y < 20) {
    document.write(`<div>Block - ${y}</div>`);
    y++;
}
...
<img src="${product.image}" alt="${product.title}" class="prouct-image">
</div>`)
}
document.write(`</div>`);
...
for (const user of users) {
    if (user.status) {
        document.write(`<li>${user.name}, age - ${user.age}. Status: ${user.status}.</li>`)
    }
}
document.write(`</ol>`);
